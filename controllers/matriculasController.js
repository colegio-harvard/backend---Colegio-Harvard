...
const AUTORIZACIONES = {
  tratamiento_datos: 'He sido informado sobre el tratamiento de los datos personales del representante legal y del estudiante que resulten necesarios para la gestión educativa, administrativa, contractual y de cobranza. Los datos se conservarán durante la relación contractual y los plazos legales aplicables. Puedo ejercer mis derechos de acceso, rectificación, cancelación y oposición mediante los canales oficiales del colegio.',
  comunicaciones_contractuales: 'He sido informado de que el colegio utilizará el domicilio, teléfono y correo electrónico registrados para enviar comunicaciones necesarias relacionadas con la matrícula y el servicio educativo.',
  ...
  try {
    ...
    const datos = await prisma.$queryRawUnsafe(`
      SELECT al.id,al.codigo_alumno,al.nombre_completo,al.apellido_paterno,al.apellido_materno,al.nombres,al.dni,al.monto_matricula,al.monto_materiales,al.monto_pension,
        p.id id_padre,p.nombre_completo apoderado,p.apellido_paterno apoderado_apellido_paterno,p.apellido_materno apoderado_apellido_materno,p.nombres apoderado_nombres,p.dni dni_apoderado,p.celular,
        g.nombre grado,a.seccion,n.nombre nivel
      FROM "tbl_alumnos" al
    ...
    const snapshot = { alumno: { id: alumno.id, codigo: alumno.codigo_alumno, nombre: alumno.nombre_completo, apellido_paterno: alumno.apellido_paterno, apellido_materno: alumno.apellido_materno, nombres: alumno.nombres, dni: alumno.dni, nivel: alumno.nivel, grado: alumno.grado, seccion: alumno.seccion, monto_matricula: Number(alumno.monto_matricula || 0), monto_pension: Number(alumno.monto_pension || 0), monto_materiales: Number(alumno.monto_materiales || 0) }, apoderado: { id: alumno.id_padre, nombre: alumno.apoderado, apellido_paterno: alumno.apoderado_apellido_paterno, apellido_materno: alumno.apoderado_apellido_materno, nombres: alumno.apoderado_nombres, dni: alumno.dni_apoderado, celular: alumno.celular }, anio: anio.anio };
...
async function aceptar(req, res) {
  try {
    ...
    if (autorizacionesFaltantes.length) return res.status(400).json({ error: 'Debe confirmar todos los apartados requeridos para formalizar la matrícula' });
...
async function guardarBorradorAsistido(req, res) {
  try {
    ...
    if (borrador.tipo_ingreso === 'PROMOCION_INTERNA') borrador.institucion_procedencia = 'Colegio Harvard';
...
async function guardarComplementoAdministrativo(req, res) {
  try {
    const matriculaId = Number(req.params.id);
    if (!Number.isInteger(matriculaId) || matriculaId <= 0) return res.status(400).json({ error: 'Matrícula inválida' });
