...
function crearMensaje({ canal, colegio, alumno, mes, saldo, conceptos, telefonoContacto }) { if (!CANALES.has(canal)) throw new Error('Canal no válido'); const seleccion = conceptos?.length ? conceptos : [{ clave_mes: mes, saldo }]; const detalle = seleccion.map((x) => `${x.concepto || x.clave_mes}: S/${formatoMonto(x.saldo)}`).join(', '); const total = seleccion.reduce((s, x) => s + Number(x.saldo || 0), 0); if (canal === 'SMS') return `${colegio}: ${alumno}, conceptos pendientes: ${detalle}. Total S/${formatoMonto(total)}. Si ya pagó, ignore este SMS. Consultas: ${telefonoContacto || ''}`.trim(); return `Estimado(a) apoderado(a), le recordamos los conceptos seleccionados pendientes de ${alumno}: ${detalle}. Total: S/${formatoMonto(total)}. Si ya realizó el pago, ignore este mensaje o envíenos su comprobante. ${colegio}.`; }
